...
class MockDreamClientTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def assert_roof_status(self, roof_status: common.RoofStatus) -> None:
    ...
    async def test_open_and_close_roof(self) -> None:
        await self.assert_roof_status(common.RoofStatus.CLOSED)

        await self.mock_dream_client.run_command(command="openRoof")
        await self.assert_server_status()
        await self.assert_command_response(command_response=common.CommandResponse.ACK)
        self.expected_status.state = common.ServerState.OPEN
        self.expected_status.roof_status = common.RoofStatus.OPEN
        await self.assert_server_status()
        await self.assert_command_response(command_response=common.CommandResponse.LAST)
